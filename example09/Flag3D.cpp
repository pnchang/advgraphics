...
Flag3D::Flag3D(LPDIRECT3DDEVICE8 dev)
{
   ...
   if (FAILED(m_vertBuffer->Lock(0, sizeof(verts), (BYTE**) &pVertices, 0 )))
   ...
   m_device->CreateIndexBuffer(sizeof(triIndex), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
      &(m_indexBuffers[0]));
   if (FAILED(m_indexBuffers[0]->Lock(0, sizeof(triIndex), (BYTE**) &pIndices, 0)))
      return;
   ...
   m_device->CreateIndexBuffer(sizeof(lineIndex), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
      &(m_indexBuffers[1]));
   if (FAILED(m_indexBuffers[1]->Lock(0, sizeof(lineIndex), (BYTE**) &pIndices, 0)))
      return;
   ...
   m_device->CreateIndexBuffer(sizeof(pointIndex), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
      &(m_indexBuffers[2]));
   if (FAILED(m_indexBuffers[2]->Lock(0, sizeof(pointIndex), (BYTE**) &pIndices, 0)))
      return;
...
void Flag3D::SetTexture(int num, LPDIRECT3DTEXTURE8 tex)
{
   if (num <= 2)
      m_textures[num] = tex;
   else
...
void Flag3D::render(DWORD curTime)
{
   ...
   m_vertBuffer->Lock(0, LENGTH * WIDTH * sizeof(CUSTOMVERTEX), (BYTE**) &ptr, 0);
   ...
   m_device->SetTextureStageState( 0, D3DTSS_ADDRESSU,  D3DTADDRESS_MIRROR );
   m_device->SetTextureStageState( 0, D3DTSS_ADDRESSV,  D3DTADDRESS_MIRROR );
   ...
   m_device->SetTextureStageState( 1, D3DTSS_ADDRESSU,  D3DTADDRESS_MIRROR );
   m_device->SetTextureStageState( 1, D3DTSS_ADDRESSV,  D3DTADDRESS_MIRROR );
   ...
   m_device->SetIndices(m_indexBuffers[m_primitiveType], 0);
   if (m_primitiveType == 0)
      m_device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0 , LENGTH * WIDTH,
         0, (LENGTH - 1) * (WIDTH - 1) * 2);
   else if (m_primitiveType == 1)
      m_device->DrawIndexedPrimitive(D3DPT_LINELIST, 0, LENGTH * WIDTH,
         0, (LENGTH - 1) * (WIDTH) + (WIDTH - 1) * (LENGTH));
   else
      m_device->DrawIndexedPrimitive(D3DPT_POINTLIST, 0, LENGTH * WIDTH, 0, LENGTH * WIDTH);
   m_device->EndScene();
}
